...
export default async function upgradeToCypress16(
  ...
  const cypressRange = getCypressRange(tree);
  if (!cypressRange || !validRange(cypressRange)) {
    const reason = cypressRange
      ? `\`cypress\` is declared as \`${cypressRange}\` in package.json, which this migration cannot bump`
...
function getCypressRange(tree: Tree): string | null {
  try {
    return getDependencyVersionFromPackageJson(tree, 'cypress');
  } catch {
    // An unresolvable catalog reference; reported as the raw declaration.
    const { dependencies = {}, devDependencies = {} } = readJson(
      tree,
      'package.json'
    );
    return devDependencies.cypress ?? dependencies.cypress ?? null;
  }
}
