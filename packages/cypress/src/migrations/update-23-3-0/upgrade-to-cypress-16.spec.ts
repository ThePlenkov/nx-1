...
`
  ...
  it.each([
    ['not declared in the root package.json', undefined],
    ['declared as a non-range specifier', 'catalog:'],
  ])(
    'should skip with a message when cypress is %s',
    async (_, cypressRange) => {
      updateJson(tree, 'package.json', (json) => {
        if (cypressRange) {
      ...
      expect(result.nextSteps).toEqual([
        expect.stringContaining(
          `Move it to \`^16.0.0\` by hand, install, and run \`${REPLAN_COMMAND} && npx nx migrate --run-migrations\``
        ),
      ]);
    }
  );
