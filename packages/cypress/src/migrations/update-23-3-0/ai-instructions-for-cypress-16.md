...
- Vite: Cypress 16 component testing runs on Vite 8 only, and `@cypress/vite-dev-server` 8 declares it as a peer. The deterministic migration kept the workspace on Cypress 15 when a Vite component testing project resolved a Vite below 8, so a workspace reaching this step runs its Vite component tests on Vite 8. Bump any `@vitejs/*` plugin that still pins an older major.
